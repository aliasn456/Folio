...
[ExecuteInEditMode]
public class DK_RPG_SelfGenerator : MonoBehaviour {	
...
//	public int maxPixels;
	...
	public DK_RPG_SelfGeneratorCoroutine umaGeneratorCoroutine;
	...
	*/
	public void Awake () {
		...
		if(_DKUMAGenerator.usePRO && !UnityEditorInternal.InternalEditorUtility.HasPro()){
			...
			else _DKUMAGenerator.usePRO = true;
		#endif
	...
	//		umaDirtyList.Clear();
		...
		if( _DKUMAGenerator.atlasResolution == 0 ) _DKUMAGenerator.atlasResolution = 256;
...
// Added by DK 
	...
	public void UpdateEditor () {
		...
		try {
		_DKUMAGenerator.meshUpdates = 0;
		}catch(NullReferenceException ){ /* Debug.LogException ("To verify : "+e);*/ }
	}	
	#endif	
...
// end Added by DK
		...
		if(umaData && umaData.isMeshDirty || umaData && umaData.isTextureDirty || umaData && umaData.isShapeDirty ){

			UpdateUMABody(umaData);
			if(umaData && umaData.isMeshDirty ) umaData.isMeshDirty = false;
			if(umaData && umaData.isTextureDirty) umaData.isTextureDirty = false;
			if(umaData && umaData.isShapeDirty) umaData.isShapeDirty = false;
	...
	public virtual void UMAReady(){	
		if(umaData){
		//	umaData.myRenderer = umaData.TMPmyRenderer;

			// verify if all textures are ready
			if ( umaData.myRenderer.sharedMaterials.Length > 0 ){
				if ( umaData.myRenderer.sharedMaterial.mainTexture == null ) {
					Debug.Log ("GetTexture (Specular) error, regenerating");
		...
		//	_DK_Model.path = ( AvatarSavePath +"/"+ umaData.transform.parent.name + ".Avatar" ) ;
			...
			try {
				...
				DestroyImmediate ( textureMerge.gameObject );
			}catch (MissingReferenceException){}
			if ( _DK_UMACrowd.RaceAndGender.MultiRace == "" || _DK_UMACrowd.RaceAndGender.MultiRace == "Random for One" ) 
				_DK_UMACrowd.RaceAndGender.Race = "Random";
		...
		//	if (animator) Object.DestroyImmediate(animator);
			...
			umaData.Creating = false;
			DestroyImmediate ( this );
		}
    }

	void CorrectEyelash (){
		foreach ( Material _material in umaData.myRenderer.gameObject.GetComponent<SkinnedMeshRenderer>().sharedMaterials ){
			if ( _material.name == "TransparentTwoSided" ) _material.color = Color.black;
		}
	}
    ...
    public virtual void UpdateUMABody (DKUMAData umaData){
		if(umaData)
        {
			...
			animationController = _DKUMAGenerator.animationController;
	...
	//	Vector2[] originalUVs = umaData.TMPmyRenderer.sharedMesh.uv;
		...
		for(int i = 0; i < atlasUVs.Length; i++ ) {
			...
			atlasUVs[i].x = Mathf.Lerp( tempAtlasRect[rectIndex].xMin/_DKUMAGenerator.atlasResolution, tempAtlasRect[rectIndex].xMax/_DKUMAGenerator.atlasResolution, originalUVs[i].x );
			atlasUVs[i].y = Mathf.Lerp( tempAtlasRect[rectIndex].yMin/_DKUMAGenerator.atlasResolution, tempAtlasRect[rectIndex].yMax/_DKUMAGenerator.atlasResolution, originalUVs[i].y );            
	...
	//	umaData.TMPmyRenderer.sharedMesh.uv = atlasUVs;	
	}	
}