...
[CustomEditor(typeof(DKSlotLibrary))]
[CanEditMultipleObjects]
public class DKSlotLibraryEditor : Editor
{
	private SerializedObject m_Object;
	private DKSlotLibrary slotLibrary;
	...
	public void OnEnable()
	{
		...
		slotLibrary = m_Object.targetObject as DKSlotLibrary;
		m_SlotDataCount = m_Object.FindProperty(kArraySizePath);
	}



	private DKSlotData[] GetSlotDataArray()
	{

		int arrayCount = m_SlotDataCount.intValue;
		DKSlotData[] SlotDataArray = new DKSlotData[arrayCount];

		for (int i = 0; i < arrayCount; i++)
		{

			SlotDataArray[i] = m_Object.FindProperty(string.Format(kArrayData, i)).objectReferenceValue as DKSlotData;
	...
	private void SetSlotData(int index, DKSlotData slotElement)
	{
		m_Object.FindProperty(string.Format(kArrayData, index)).objectReferenceValue = slotElement;
		isDirty = true;
	}

	private DKSlotData GetSlotDataAtIndex(int index)
	{
		return m_Object.FindProperty(string.Format(kArrayData, index)).objectReferenceValue as DKSlotData;
	}

	private void AddSlotData(DKSlotData slotElement)
	{
		...
		if (evt.type == EventType.DragPerform)
		{
			if (dropArea.Contains(evt.mousePosition))
			{
				...
				for (int i = 0; i < draggedObjects.Length; i++)
				{
					if (draggedObjects[i])
					{
						DKSlotData tempSlotData = draggedObjects[i] as DKSlotData;
						if (tempSlotData)
						{
						...
						if (System.IO.Directory.Exists(path))
						{
							...
							{
								tempSlotData = AssetDatabase.LoadAssetAtPath(assetFile, typeof(DKSlotData)) as DKSlotData;
								if (tempSlotData)
								{
									AddSlotData(tempSlotData);
								}
	...
	public override void OnInspectorGUI()
	{
		...
		DKSlotData[] slotElementList = GetSlotDataArray();
		...
		if (GUILayout.Button("Order by Name"))
		{
			...
			List<DKSlotData> SlotDataTemp = slotElementList.ToList();
		...
		for (int i = 0; i < m_SlotDataCount.intValue; i++)
		{
			...
			DKSlotData result = EditorGUILayout.ObjectField(slotElementList[i], typeof(DKSlotData), true) as DKSlotData;
