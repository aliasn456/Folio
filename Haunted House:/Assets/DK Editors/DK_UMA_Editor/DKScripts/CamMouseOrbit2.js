...
function LateUpdate () 
{
 //   if (!target) return;
	if (!target && ModelName) {
		if ( ModelName.text == "" || ModelName.text == null ) ModelName.text == "Hips";
	//	if ( target ) target = GameObject.Find("Hips").transform;
		
		
		
		if ( ModelName ){
			var tmpobj;
			tmpobj = GameObject.Find(ModelName.text);
			if (tmpobj) target = tmpobj.transform;
		}
		return;
	}
	if ( ModelName ) target = GameObject.Find(ModelName.text).transform;
