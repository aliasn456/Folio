...
public class DK_DefineSlot : MonoBehaviour {
...
# if Editor
									Crowd.tempSlotList[Crowd.tempSlotList.Count-1].Preview = TmpTorsolist[Ran].Preview;
# endif
...
# if Editor
									Crowd.tempSlotList[Crowd.tempSlotList.Count-1].Preview = TmpMeshlist[Ran].Preview;
#endif
							...
							if ( TmpHairlist.Count > 0 ) {
								    ...
								    )
								{
									    ...
									    && AssignedSlotsList.Contains(TmpHairlist[Ran]) ==  false ) 
									{
										...
										# if Editor
										Crowd.tempSlotList[Crowd.tempSlotList.Count-1].Preview = TmpHairlist[Ran].Preview;
# endif
							...
							if ( Crowd.tempSlotList[Crowd.tempSlotList.Count-1].OverlayType == "Hair"
								...
								TmpLinkedToSlot = Crowd.tempSlotList[Crowd.tempSlotList.Count-1].overlayList;
						    ...
						    && RanSpawn <= Crowd.Anato_DK_Race.SpawnPerct )
						{
							...
							for(int i1 = 0; i1 < Crowd.slotLibrary.slotElementList.Length; i1 ++){
									    ...
									    && ( Crowd.Slot_DK_Race.Gender == Crowd.RaceAndGender.Gender || Crowd.Slot_DK_Race.Gender == "Both" ) )
								{
									...
									else if ( Crowd.Slot_DK_Race.OverlayType.Contains("Wear") == false ) {
...
# endif
							    ...
							    && Crowd.CurrentSlot.OverlayType != "Face" )
							{
								...
								if ( Crowd.CurrentSlot.OverlayType == "LegsWear" ) Crowd.Colors.ColorToApply =Crowd.Colors.LegsWearColor;
								if ( Crowd.CurrentSlot.OverlayType == "HandWear" ) Crowd.Colors.ColorToApply =Crowd.Colors.HandWearColor;
								if ( Crowd.CurrentSlot.OverlayType == "FeetWear" ) Crowd.Colors.ColorToApply =Crowd.Colors.FeetWearColor;
								if ( Crowd.CurrentSlot.OverlayType == "ShoulderWear" ) Crowd.Colors.ColorToApply =Crowd.Colors.TorsoWearColor;
										    ...
										    && ( Crowd.Slot_DK_Race.Place ==Crowd.Ov_DK_Race.Place || Crowd.Anato_DK_Race.Place ==Crowd.Ov_DK_Race.Place ) )
									{
										...
										if ( Crowd.Slot_DK_Race.LinkedOverlayList.Count == 0 ) {
											...
											if ( Crowd.Wears.WearOverlays
											    ...
											    &&Crowd.Ov_DK_Race.OverlayType != ("Eyes") 
											    && Crowd.Slot_DK_Race.OverlayType != ("Hair")
											    && Crowd.Slot_DK_Race.OverlayType == ("Flesh")
											    && Crowd.Slot_DK_Race.Active == true
											    &&Crowd.Ov_DK_Race.OverlayType != ("Face")
		...
		for(int i3 = 0; i3 < Crowd.tempSlotList.Count; i3 ++){
			#region if Replace activated
			if ( Crowd.tempSlotList[i3].Replace == true ) {
				for(int i1 = 0; i1 < Crowd.tempSlotList.Count; i1 ++){
					if ( Crowd.tempSlotList[i3].Place.dk_SlotsAnatomyElement.Place == Crowd.tempSlotList[i1].Place ) {
						Crowd.tempSlotList.Remove(Crowd.tempSlotList[i1]);
					}
				}
			}
			...
			if ( Crowd.tempSlotList[i3]._HideData.HideShoulders ){
				Crowd.Wears.HideShoulders = true;
			}
			// detect the shoulders
			if ( Crowd.tempSlotList[i3].Place.name == "ShoulderWear" ){
				Crowd.Wears.Shoulders = Crowd.tempSlotList[i3];
			}
			#endregion hide shoulders
			
			#region hide LegWear
			// detect 'hide Legs'
			if ( Crowd.tempSlotList[i3]._HideData.HideLegs ){
				Crowd.Wears.HideLegs = true;
			}
			// detect the shoulders
			if ( Crowd.tempSlotList[i3].Place.name == "LegsWear" ){
				Crowd.Wears.Legs = Crowd.tempSlotList[i3];
			}
		...
		//	if ( UseHeadLook ) umaData.AddHeadLook ();
		//	if ( UseEyeBlink ) umaData.AddEyeBlink ();
		#endregion Finishing
	}
